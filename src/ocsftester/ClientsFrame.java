...
/**
 * The <code> ClientsFrame </code> class is an application that randomly
 * generates clients, connects them to the server, makes them sending messages,
 * and closes them. Up to 25 clients can be present at a given time, these
 * occupy one of the 25 panels included in the window. Type
 * <code>java ocsftester.ClientsFrame host port_number</code> to start the
 * application.
 * <p>
 * A panel is pink when the connection has been closed, red, orange or light
 * gray when an exception is received, blue when it tries to connect to the
 * server, and green when connected to the server.
 * 
...
public class ClientsFrame extends Frame implements Runnable {
	...
	/**
	 * Starts the client generator. The default host is localhost. The default
	 * port is 12345.
	 */
